(function () {
    ...
    ko.unpromise = function (evaluator, options) {
        ...
        ko.execute(result, function () {
            ...
            var w = ++waitingOn;
            // test whether the previous value is a cancellable promise
            if (prev && prev !== p) {
                // don't fail if the cancellation fails
                try {
                    if (typeof prev.cancel === "function")
                        prev.cancel();
                    else if (typeof prev.abort === "function")
                        prev.abort();
                } catch (e) {
                }
            }
            ...
            if (p && typeof p.then === "function") {
                ...
                p.then(function (v) {
                    if (w === waitingOn) {
                        prev = null; // no more cancellation attempts
                        latest(v);
                    }
                }, function () {
                    if (w === waitingOn) {
